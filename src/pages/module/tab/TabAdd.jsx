...
const TabAdd = () => {
  ...
  const handleChangeSubTab = (id, value) => {
    setSubTabs(
      subTabs.map((sub) => (sub.id === id ? { ...sub, name: value } : sub))
    );
  };
    ...
    <div className="flex flex-col font-medium text-[#0D2E28] p-2 h-full font-[Poppins]">
      <div className="flex items-center bg-white border rounded-lg shadow p-3 mb-4">
        <img
          ...
          alt="Back"
        />
        <h2 className="text-lg font-semibold text-gray-800">Add New Tab</h2>
      </div>
        ...
        >
          ...
          <div className="flex justify-end">
            <button
              ...
              className="w-[163px] h-[24px] font-[Poppins] font-medium text-[16px] leading-[24px] text-[#001580] text-center hover:underline"
            >
              + Add More Sub Tab
            </button>
          </div>
