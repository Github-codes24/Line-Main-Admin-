  ...
  return (
    <div className="bg-[#E0E9E9] min-h-screen w-full">
      ...
      <div className="bg-white p-4 shadow rounded-md">
                  ...
                  {currentRecords.length === 0 ? (
                    ...
                    currentRecords.map((c, idx) => (
                      <tr key={c._id} className="text-center">
                        ...
                        <td className="w-[200px] h-[56px] align-middle">{c.categoryName || "N/A"}</td>
                        ...
                        <td className="w-[140px] h-[56px] align-middle">
                          <div className="flex justify-center space-x-4 flex-wrap">
                            <button onClick={() => navigate(`/admin/set-commission/view/${c._id}`)} className="text-[#EC2D01] hover:text-orange-700">
                              <Eye size={20} />
                            </button>
                            <button onClick={() => navigate(`/admin/set-commission/edit/${c._id}`)} className="text-[#EC2D01] hover:text-orange-700">
                              <Edit size={20} />
                            </button>
                            <button onClick={() => setSelectedCommission(c)} className="text-[#EC2D01] hover:text-orange-700">
                              <Trash2 size={20} />
                            </button>
                          </div>
                        </td>
        ...
        {selectedCommission && (
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
            <div className="bg-white rounded-lg shadow-lg w-[90%] max-w-md p-6">
              <h2 className="text-xl font-bold text-center text-[#0D2E28] mb-3">Delete Commission</h2>
              <p className="text-[#0D2E28] text-center mb-6 leading-relaxed">Are you sure you want to delete this commission?</p>
              <div className="flex justify-center gap-4 flex-wrap">
                <button onClick={() => setSelectedCommission(null)} className="px-16 py-2 rounded-md border border-[#001580] bg-[#CED4F2] text-[#001580] font-medium hover:opacity-90 transition w-full sm:w-auto">Cancel</button>
                <button onClick={() => { deleteCommission(selectedCommission._id); setSelectedCommission(null); }} className="px-16 py-2 rounded-md border border-[#001580] bg-[#001580] text-white font-medium hover:opacity-90 transition w-full sm:w-auto">Delete</button>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Pagination */}
        <Pagination currentPage={currentPage} totalRecords={totalRecords} recordsPerPage={recordsPerPage} goToPage={goToPage} label="entries" />
      </div>
    </div>
  );
};
