...
const [selectedProduct, setSelectedProduct] = useState(null);
  ...
  return (
    ...
    // <div className="w-full min-h-screen p-4 font-[Poppins] ">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4 bg-white px-4 py-3 rounded-lg shadow">
        <h2 className="text-xl text-[black] font-medium">Small Product List</h2>
  ...
  <div className="relative flex flex-wrap gap-2 mb-4">
    ...
    >
      ...
      {/* Icon stroke color changed to #0D2E28 */}
      <path
    ...
    {showFilterPanel && (
      <div className="absolute left-0 top-14 bg-white rounded-lg shadow-lg px-4 py-2 w-70 border border-gray-300 z-50">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setShowFilterPanel(false)} className="text-[#0D2E28] hover:text-gray-800 text-2xl">
            <IoClose />
          </button>
        </div>
        <div className="space-y-3">
          {expertiseList.map((item) => (
            <label key={item} className="flex items-center space-x-2 cursor-pointer text-[#0D2E28]">
              <input
                type="checkbox"
    ...
    {activeFilters.map((filter) => (
      <span key={filter} className="px-3 py-1 bg-[#E4E5EB] text-[#0D2E28] font-medium rounded-full flex items-center gap-1">
        {filter}
        <button onClick={() => removeFilter(filter)} className="text-[#0D2E28] font-semibold ml-1">
          <IoClose />
        </button>
      </span>
    ))}
  ...
  <thead className="bg-[#E4E5EB] text-black text-base font-medium">
    {/* <tr> */}
              ...
              {paginatedData.map((product, idx) => (
                <tr>
                  ...
                  <td className="p-3 flex justify-center">
                    <img src={product.productImageUrl} alt="Product" className="w-14 h-14 rounded border p-0.5" />
                  </td>
  ...
  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
    <div className="bg-white rounded-lg shadow-lg w-[90%] max-w-md p-6">
      ...
      <div className="flex justify-center gap-4">
        ...
        <button
          onClick={() => {
            handleDelete(selectedProduct._id); // call your delete function
        ...
        <div className="flex flex-col md:flex-row items-center justify-between bg-gray-200 mt-5 rounded-lg shadow text-sm text-gray-700 gap-4 py-4 px-6">
          ...
          <div className="flex items-center space-x-2">
            <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className="px-2 py-1 bg-white border rounded disabled:opacity-50">
              &lt;
            </button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((pg) => (
              <button
                key={pg}
                onClick={() => goToPage(pg)}
                className={`w-8 h-8 border rounded ${pg === currentPage ? "bg-[#001580] text-white" : "bg-[#CECEF2] text-black"}`}
              >
                {pg}
              </button>
            ))}
            <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} className="px-2 py-1 bg-white border rounded disabled:opacity-50">
              &gt;
            </button>
          </div>
        </div>
